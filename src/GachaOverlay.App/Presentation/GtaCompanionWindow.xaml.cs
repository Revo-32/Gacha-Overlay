...
public partial class GtaCompanionWindow : Window
{
    private double _surfaceOpacity = HudSettingsDefaults.SurfaceOpacity;

    public GtaCompanionWindow()
    {
        InitializeComponent();
        RefreshTheme();
    }
    ...
    public void ApplyChallengeOnlyLayout(bool challengeOnly)
    {
        if (!IsLoaded) return;
        Height = challengeOnly ? Math.Min(Height, 355) : Math.Max(Height, 470);
    }

    public void SetSurfaceOpacity(double opacity)
    {
        _surfaceOpacity = HudSettingsDefaults.NormalizeSurfaceOpacity(opacity);
        RefreshTheme();
    }
    ...
    public void RefreshTheme()
    {
        ...
        Surface.Background = ColorThemeManager.CreateOpacityBrush(
            SemanticColorToken.SurfaceBase,
            alpha);
        Surface.BorderBrush = ColorThemeManager.CreateOpacityBrush(
            SemanticColorToken.BorderSubtle,
            alpha == 0 ? (byte)0 : (byte)Math.Max(28, alpha / 2));
        Resources["GtaCompanionCardSurfaceBrush"] = ColorThemeManager.CreateOpacityBrush(
