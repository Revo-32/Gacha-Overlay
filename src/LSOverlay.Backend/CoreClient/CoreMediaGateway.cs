...
internal sealed class CoreMediaGateway : IDisposable
{
    ...
    public CoreMediaGateway() : this(new SocketsHttpHandler { AllowAutoRedirect = false, UseCookies = false, UseProxy = false, MaxConnectionsPerServer = 8 },
    ...
    public async Task Serve(HttpContext context, CoreMediaScope scope, string id, int width, int height)
    {
        ...
        finally { _slots.Release(); scope.Leave(); }
    }
