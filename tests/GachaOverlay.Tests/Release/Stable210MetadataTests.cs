...
public sealed class Stable210MetadataTests
{
    ...
    public void ReleaseMetadataMatchesManifestWithoutRenamingManagedAssembly()
    {
        using var manifest = JsonDocument.Parse(
            File.ReadAllText(Path.Combine(Root, "tools/release/ls-2.1.0.json")));
        ...
        Assert.Equal("LS-Overlay-2.1.0-win-x64.zip",
        ...
        Assert.Equal("LS-Overlay-2.1.0-SHA256.txt",
            manifest.RootElement.GetProperty("checksumName").GetString());
        Assert.Equal("v2.1.0", manifest.RootElement.GetProperty("tag").GetString());
        Assert.Equal("LS Overlay 2.1.0",
