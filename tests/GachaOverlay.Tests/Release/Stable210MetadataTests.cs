...
public sealed class Stable210MetadataTests
{
    ...
    public void ReleaseMetadataMatchesManifestWithoutRenamingManagedAssembly()
    {
        using var manifest = JsonDocument.Parse(
            File.ReadAllText(Path.Combine(Root, "tools/release/ls-2.3.0.json")));
        ...
        Assert.Equal("LS-Overlay-2.3.0-win-x64.zip",
            manifest.RootElement.GetProperty("zipName").GetString());
        Assert.Equal("LS-Overlay-2.3-Quick-Start-ko.pdf",
            manifest.RootElement.GetProperty("quickStartName").GetString());
        Assert.Equal("LS-Overlay-2.3-User-Guide-ko.pdf",
            manifest.RootElement.GetProperty("guideName").GetString());
        Assert.Equal("LS-Overlay-2.3.0-SHA256.txt",
            manifest.RootElement.GetProperty("checksumName").GetString());
        Assert.Equal("v2.3.0", manifest.RootElement.GetProperty("tag").GetString());
        Assert.Equal("LS Overlay 2.3.0",
    ...
    public void PublicDocumentsCarryCurrentVersionContactAndLinksWithoutLegacySetup()
    {
        foreach (var relative in new[]
                 {
                     "README.md",
                     "docs/2.3/quick-start/LS-Overlay-2.3-Quick-Start-ko.md",
                     "docs/2.3/user-guide/LS-Overlay-2.3-User-Guide-ko.md",
                     "docs/releases/LS-Overlay-2.3.0-release-notes.md",
                 })
        {
            var text = File.ReadAllText(Path.Combine(Root, relative));
            Assert.Contains("2.3.0", text);
    ...
    public void TwoGuideSourcesUseCurrentSharedFactsAndExplicitScreenshotGate()
    {
        var quick = File.ReadAllText(Path.Combine(
            Root, "docs/2.3/quick-start/LS-Overlay-2.3-Quick-Start-ko.md"));
        var full = File.ReadAllText(Path.Combine(
            Root, "docs/2.3/user-guide/LS-Overlay-2.3-User-Guide-ko.md"));
