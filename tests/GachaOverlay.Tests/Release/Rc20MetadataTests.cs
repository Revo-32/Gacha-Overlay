...
public sealed class Rc20MetadataTests
{
    ...
    {
        using var manifest = JsonDocument.Parse(File.ReadAllText(Path.Combine(Root, "tools/release/ls-2.0.0-rc.1.json")));
        ...
        foreach (var relative in new[]
                 {
                     ...
                     "docs/releases/LS-Overlay-2.0.0-rc.1.md", "docs/releases/LS-Overlay-2.0.0-rc.1-github-release.md",
                 })
        {
            var text = File.ReadAllText(Path.Combine(Root, relative));
            Assert.Contains("2.0.0-rc.1", text);
