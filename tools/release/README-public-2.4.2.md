...
2.4.2는 GTA 컴패니언의 주간 보너스·할인을 같은 배율과 할인율별로 묶어 반복 제목을 줄인 클라이언트 UI 패치입니다. GTA$·RP와 속도 보너스, 일반 할인과 GTA+ 회원 전용 할인은 구분합니다. 기존 항목과 조건은 유지하며, 이 변경을 보려면 새 ZIP으로 앱을 업데이트해야 합니다.
...
두 PDF는 승인된 2.4 계열 문서를 그대로 재사용합니다. 문서의 2.4.0 파일명 예시는 이번 릴리즈의 `LS-Overlay-2.4.2-win-x64.zip`과 `LS-Overlay-2.4.2-SHA256.txt`로 읽어 주세요. 조작 방법은 동일합니다.
