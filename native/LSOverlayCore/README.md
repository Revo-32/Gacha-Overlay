...
판매 완료·완료 취소를 포함한 최신 검증 실행본은 `m7-sales-polish`입니다. 확인창 없이 클릭 즉시 실행하며 버튼 글자는 가로·세로 중앙 정렬입니다. 판매 안내 문구는 왼쪽 위치를 유지하면서 세로 중앙 정렬하고, 잠금 상태에서는 판매 조작 버튼과 접기 화살표를 숨깁니다. [판매 조작 검증 기록](M7-SALES.md)에 검증 범위를 구분했습니다. `launch-sales-core.ps1`로 실행하며 기존 읽기 전용 launcher의 기본 동작은 보존합니다.

## M1 당시 기록

아래는 보존된 초기 M1 shell 설명입니다. 기능 미구현·설정 미저장 등의 문구는 당시 상태이며 현재 상태는 위 문서들을 기준으로 확인해야 합니다.
