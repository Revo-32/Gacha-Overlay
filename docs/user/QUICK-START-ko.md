...
1. `LS-Overlay-2.0.0-win-x64.zip`을 다운로드하고 새 폴더에 모두 압축 해제합니다. GitHub의 `Source code` 파일은 실행용이 아닙니다.
...
이 버전은 코드 서명되지 않았습니다. Windows 보안 경고가 나타나면 배포 출처와 제공된 SHA-256을 먼저 확인하세요. 보안 프로그램을 끄지 마세요.
...
- 자세한 사용법은 함께 제공된 **LS-Overlay-2.0-User-Guide-ko.pdf**를 확인하세요.
