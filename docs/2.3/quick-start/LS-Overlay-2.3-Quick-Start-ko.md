...
GTA Online 화면 위에서 Discord 채팅, 판매 순서와 2.3의 보조 HUD를 빠르게 시작하는 안내서입니다.

**대상:** Windows x64 · LS Overlay 2.3.0

이미지는 유지된 조작을 설명하는 이전 UI 예시입니다. 2.3의 변경된 알림·판매·감지 동작은 본문을 따릅니다.
...
1. 정식 배포 후 [GitHub Releases](https://github.com/Revo-32/Gacha-Overlay/releases)에서 `LS-Overlay-2.3.0-win-x64.zip`을 받습니다.
2. GitHub가 자동 생성한 `Source code`가 아니라 이름이 정확히 일치하는 Windows x64 ZIP인지 확인합니다.
3. 다운로드한 ZIP의 SHA-256을 릴리즈 페이지의 `LS-Overlay-2.3.0-SHA256.txt`와 비교합니다.
...
LS Overlay 2.3.0은 코드 서명된 설치 프로그램이 아니므로 Windows SmartScreen 또는 보안 프로그램의 확인 화면이 나타날 수 있습니다.
...
```powershell
Get-FileHash .\LS-Overlay-2.3.0-win-x64.zip -Algorithm SHA256
```
