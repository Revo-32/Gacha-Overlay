...
GTA Online 화면 위에서 Discord 채팅, 판매 순서와 2.4의 보조 HUD를 빠르게 시작하는 안내서입니다.

**대상:** Windows x64 · LS Overlay 2.4.0

이미지는 유지된 조작을 설명하는 이전 UI 예시입니다. 현재 동작과 AI 번역 범위는 본문을 따릅니다.
...
1. 정식 배포 후 [GitHub Releases](https://github.com/Revo-32/Gacha-Overlay/releases)에서 `LS-Overlay-2.4.0-win-x64.zip`을 받습니다.
2. GitHub가 자동 생성한 `Source code`가 아니라 이름이 정확히 일치하는 Windows x64 ZIP인지 확인합니다.
3. 다운로드한 ZIP의 SHA-256을 릴리즈 페이지의 `LS-Overlay-2.4.0-SHA256.txt`와 비교합니다.
...
LS Overlay 2.4.0은 코드 서명된 설치 프로그램이 아니므로 Windows SmartScreen 또는 보안 프로그램의 확인 화면이 나타날 수 있습니다.
...
```powershell
Get-FileHash .\LS-Overlay-2.4.0-win-x64.zip -Algorithm SHA256
```
...
- **알림센터:** F10으로 잠금을 푼 뒤 설정 왼쪽 종 버튼을 누릅니다. 멘션·답장·판매 차례·GTA 감지 알림을 확인하고 개별 또는 모두 읽음으로 처리합니다.
- **멘션 배경:** 설정 → 채팅의 ‘나를 멘션한 메시지 배경 강조’에서 배경만 끌 수 있습니다. 알림 기록은 유지됩니다.

알림 목록은 채팅 영역을 덮고 판매 영역은 가리지 않습니다. 알림은 고정 12pt이며 배경은 항상 불투명합니다. 판매 원문은 펼친 상세에서 확인합니다. 판매 상세는 읽기 전용이며, 자신의 판매 행에서는 서버 확인을 거치는 **판매완료** 버튼을 사용할 수 있습니다.
...
Google Gemini를 이용한 AI 번역은 지정 채널·작성자가 모두 확인된 공개 GTA 온라인 이벤트 소스에만 적용됩니다. 일반 Discord 채팅·판매 메시지·사용자 정보·인증정보는 전송하지 않습니다.

검증된 번역은 서버에서 재사용합니다. AI를 사용할 수 없거나 검증에 실패하면 안전한 기존 정보 또는 원문을 표시합니다.

## 사업장 관리자

설정의 **사업장 관리자**에서 기능을 켜고 실제로 사용하는 사업장만 선택합니다. GTA 내부 재고를 직접 읽지 않으므로 실제 행동 직후 버튼을 누릅니다. 생산 시간은 로컬 GTA 실행 중 누적하며 스토리 모드도 포함합니다. Alt+Tab은 종료가 아닙니다. 컴패니언에서 감지 상태를 확인하세요.
